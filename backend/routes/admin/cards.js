...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`[GET /cards/course/${courseId}] Querying with:`, where);
      ...
      console.log(`[GET /cards/course/${courseId}] Found ${cards.length} cards`);

      // Also check total cards for this course (without userId filter)
      const totalForCourse = await Card.count({ where: { courseId } });
      console.log(`[GET /cards/course/${courseId}] Total cards for course (all userId): ${totalForCourse}`);
...
/**
 * @route   POST /api/admin/cards/fix-template-cards
 * @desc    Fix template cards by setting userId to null
 * @access  Admin only
 */
router.post(
  '/fix-template-cards',
  ...
  async (req, res) => {
    try {
      ...
      console.log('[FIX] Starting template cards fix...');
      ...
      console.log(`[FIX] Found ${cardsWithUserId.length} cards with userId set (should be null for template cards)`);

      if (cardsWithUserId.length === 0) {
        return res.json({
          success: true,
          message: 'No cards to fix!',
          data: {
            updated: 0
          }
        });
      ...
      console.log(`[FIX] Successfully updated ${updatedCount} cards to set userId = null`);

      res.json({
        success: true,
        message: `Successfully updated ${updatedCount} cards to set userId = null`,
        data: {
          updated: updatedCount
        }
      });
    } catch (error) {
      console.error('[FIX] Error fixing template cards:', error);
      res.status(500).json({
        success: false,
        error: 'Error fixing template cards'
      });
    }
  }
);
